...
def choose(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    res_list = list(filter(select, paragraphs))
    if k < len(res_list):
        return res_list[k]
    else:
        return ''
...
def about(topic):
    ...
    def match(s):
        for i in string.punctuation:
            s = s.replace(i, '')
        word_list = [lower(x) for x in s.split(' ')]
        for word in topic:
            if word in word_list:
                return True
        return False

    return lambda x: match(x)
    # END PROBLEM 2
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    if typed_word in valid_words:
        return typed_word
    diff_list = [[diff_function(typed_word, word, limit), word] for word in valid_words]
...
def time_per_word(times_per_player, words):
    ...
    return game(words,
                [
                    [times_per_player[timestamp][i] - times_per_player[timestamp][i - 1] for i in
                     range(1, len(times_per_player[timestamp]))]
                    for timestamp in range(len(times_per_player))
                ]
            )
    # END PROBLEM 9
...
def fastest_words(game):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    # ret = [[]] * len(all_times(game))       # 这里有坑
    ret = [[] for _ in player_indices]
    for i in word_indices:
        word = all_words(game)[i]
        time_list = [all_times(game)[j][i] for j in player_indices]
        nominate_player, min_time = [], min(time_list)
        for j in range(len(time_list)):
            if time_list[j] == min_time:
                nominate_player.append(j)
        # print(f'nominate for i={i}: {nominate_player}, append:{all_words(game)[i]}')
        # print(f'ret:{ret}')
        ret[nominate_player[0]].append(all_words(game)[i])
    return ret
    # END PROBLEM 10
