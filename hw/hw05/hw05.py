...
class Coin:
    ...
    def worth(self):
        "*** YOUR CODE HERE ***"
        # print(f'self year:{self.year}, cur year:{Mint.current_year}')
        if isinstance(self, Nickel):
            ret = self.cents
            if Mint.current_year > 50 + self.year:
                ret += Mint.current_year - self.year - 50
            return ret
...
def bst_min_max(t):
    ret_min, ret_max = t.label, t.label
    for b in t.branches:
        tmp_min, tmp_max = bst_min_max(b)
        ret_min = min(ret_min, tmp_min)
        ret_max = max(ret_min, tmp_max)
...
def is_bst(t):
    ...
    if t.is_leaf():
        ...
        if len(t.branches) > 1:
            right = t.branches[1]
        left_min, left_max = bst_min_max(left)
        if left_max > t.label:
            return False
        if right is not None:
            right_min, right_max = bst_min_max(right)
            if right_min <= t.label:
                return False
        flag = True
        flag &= is_bst(left)
        if right is not None:
